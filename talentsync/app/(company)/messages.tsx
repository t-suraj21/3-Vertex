...
export default function CompanyMessagesScreen() {
  ...
  const fetchInbox = useCallback(async () => {
    try {
      ...
      console.error('Company Inbox fetch error:', err);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    <View style={styles.container}>
      ...
      <SafeScreenLayout style={styles.safeArea}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Talent Messages</Text>
          <View style={{ width: 40 }} />
        </View>
        ...
        >
          ...
          {loading && !refreshing ? (
            ...
            <View style={styles.emptyState}>
              <MaterialCommunityIcons name="comment-account-outline" size={60} color="#D1D5DB" />
              <Text style={styles.emptyTitle}>No Conversations</Text>
              <Text style={styles.emptyText}>You haven't messaged any candidates yet. Go to 'Review Candidates' to start a chat!</Text>
            </View>
          ) : (
            filteredInbox.map(item => (
              ...
              >
                <View style={styles.avatarCircle}>
                  <Text style={styles.avatarText}>{(item.studentId?.name?.[0] || 'S').toUpperCase()}</Text>
                </View>
                <View style={styles.messageInfo}>
                  <Text style={styles.contactName}>{item.studentId?.name || 'Candidate'}</Text>
...
const styles = StyleSheet.create({
  ...
  avatarCircle: { width: 46, height: 46, borderRadius: 23, backgroundColor: '#E0E7FF', justifyContent: 'center', alignItems: 'center' },
